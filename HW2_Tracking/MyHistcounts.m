function [n,edges,bin] = MyHistcounts(x, varargin)
            ...
            elseif ~isempty(opts.BinWidth)
                ...
                if strcmp(opts.BinMethod, 'auto')
                    ...
                    switch opts.BinMethod
                        ...
                        case 'integers'
                            edges = integerrule(xc,minx,maxx,false);
                        case 'sqrt'
            ...
            if ~isempty(xc)
                if strcmp(opts.BinMethod, 'auto')
                    ...
                    switch opts.BinMethod
                        ...
                        case 'integers'
                            edges = integerrule(xc,minx,maxx,true);
                        case 'sqrt'
    ...
    for i = 1:2:inputlen
        ...
        switch name
            case 'NumBins'
                ...
                opts.BinWidth = [];
            case 'BinEdges'
                validateattributes(value,{'numeric'},{'vector', ...
...
xrange = maxx - minx;
if ~isempty(x) && (isinteger(x) || isequal(round(x),x))...
        && xrange <= 50 && maxx <= flintmax(class(maxx))/2 ...
        && minx >= -flintmax(class(minx))/2
    edges = integerrule(x,minx,maxx,hardlimits);
else
...
function edges = integerrule(x, minx, maxx, hardlimits)
xrange = maxx-minx;
if ~isempty(minx)
